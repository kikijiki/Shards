...
function expand_h(n, g, h)
	...
	if x > 1 then
		table.insert(ret, makeNode_h(n, g, h, x, y, x - 1, y, "left"))
	end

	if x < #g[1] then
		table.insert(ret, makeNode_h(n, g, h, x, y, x + 1, y, "right"))
	end

	if y > 1 then
		table.insert(ret, makeNode_h(n, g, h, x, y, x, y - 1, "up"))
	end

	if y < #g then
		table.insert(ret, makeNode_h(n, g, h, x, y, x, y + 1, "down"))
	end
...
function expand(n, g)
	...
	if x > 1 then
		table.insert(ret, makeNode(n, g, x, y, x - 1, y, "left"))
	end

	if x < #g[1] then
		table.insert(ret, makeNode(n, g, x, y, x + 1, y, "right"))
	end

	if y > 1 then
		table.insert(ret, makeNode(n, g, x, y, x, y - 1, "up"))
	end

	if y < #g then
		table.insert(ret, makeNode(n, g, x, y, x, y + 1, "down"))
	end
