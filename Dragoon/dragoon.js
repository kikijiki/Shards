...
function draw()
{
	...
	ctx.fillStyle = "white";
	ctx.font = "42px 'ＭＳ Ｐゴシック'";
	ctx.fillText(title, 100, 100);
	ctx.font = "16px 'ＭＳ Ｐゴシック'";
	ctx.fillText("【←　→：移動】【↑：跳ねる】【スペース：飛ぶ】", 100, 150);
}
...
function overTitle(e)
{
	...
	if(x > 100 && x < (metrics.width - 100) && y > 100 - 64 && y < (100))
	{
		return true;
	} 
...
function onMouseDown(e)
{
	if(overTitle(e))
	{
		window.open("https://www.entcomp.esys.tsukuba.ac.jp/labwiki/index.php?matteo", "_blank");
	}
}
